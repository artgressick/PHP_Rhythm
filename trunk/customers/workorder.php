<?
	...
	$q = "SELECT WorkOrders.ID, Quotes.idCustomer, WorkOrders.chrWorkOrder, WorkOrders.dtCreated, DATE_FORMAT(WorkOrders.dtCreated,'%m/%d/%Y') as dtFormated, 
		...
		WHERE Customers.ID IN (SELECT idCustomer FROM CustomerAccess WHERE idPerson=".$_SESSION['idCustomer'].") AND WorkOrders.chrKEY='". $_REQUEST['key'] ."'";
	...
	$q = "SELECT chrCustomerNumber,chrPhoneType 
		...
		JOIN PhoneTypes ON PhoneTypes.ID=CustomerNumbers.idType
