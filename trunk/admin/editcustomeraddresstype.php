<?php
	...
	if(!isset($_REQUEST['key'])) {
		 errorPage('Invalid Customer Address Type');
	}

	# Getting all original data from this record
	$info = db_query("SELECT * FROM CustomerAddressTypes WHERE chrKEY='". $_REQUEST['key'] ."'","getting Customer Address Type info",1);
	if($info['ID'] == "") { errorPage('Invalid Customer Address Type'); }
	...
	if(isset($_POST['chrCustomerAddressType'])) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		list($mysqlStr,$audit) = set_strs($mysqlStr,'chrCustomerAddressType',$info['chrCustomerAddressType'],$audit,$table,$info['ID']);
		list($mysqlStr,$audit) = set_strs($mysqlStr,'bShow',$info['bShow'],$audit,$table,$info['ID']);
		
		// if nothing has changed, don't do anything.  Otherwise update / audit.
		if($mysqlStr != '') { 
			$_SESSION['InfoMessage'] = $_POST['chrCustomerAddressType']. " has been successfully updated in the Database.";
			list($str,$aud) = update_record($mysqlStr, $audit, $table, $info['ID']);
		} else {
			$_SESSION['InfoMessage'] = "No Changes have been made to ".$info['chrCustomerAddressType'];
		}
	...
	$title = 'Edit Customer Address Type';
...
<?	
	...
	// Banner Information
	$banner_title = "Edit Customer Address Type:"; // Title of this page. (REQUIRED)
	$banner_icon = ""; // Icon for this page, Size MUST be 40x40 pixels. (NOT REQUIRED)
	$banner_xtra = $info['chrCustomerAddressType']; // Extra information for Page. (NOT REQUIRED)
		...
		<table width="100%" class="twoCol800" id="twoCol800" cellpadding="0" cellspacing="0">
			<tr>
				<td class="left">
					...
					<div class='FormName'>Customer Address Type Name <span class='FormRequired'>(Required)</span></div>
					<div class='FormField'><input type="text" name="chrCustomerAddressType" id="chrCustomerAddressType" maxlength="150" size="50" value='<?=$info['chrCustomerAddressType']?>' /></div>
