<?php
	...
	// If a post occured
	if(isset($_POST['chrPhoneType'])) { // When doing isset, use a required field.  Faster than the php count funtion.
		
		$table = 'PhoneTypes'; # added so not to forget to change the insert AND audit
		...
		$q = "INSERT INTO ". $table ." SET 
			chrKEY = '". makekey() ."',
			chrPhoneType = '". encode($_POST['chrPhoneType']) ."',
			bShow = '". $_POST['bShow'] ."',
		...
		if(db_query($q,"Insert into ". $table)) {
			...
			$q = "INSERT INTO Audit SET 
				idType=1, 
				idRecord='". $newID ."',
				txtNewValue='". encode($_POST['chrPhoneType']) ."',
				dtDateTime=now(),
				chrTableName='". $table ."',
			...
			$_SESSION['InfoMessage'] = "New Phone Type: " . encode($_POST['chrPhoneType']) . " has been added";
			header("Location: ". $_POST['moveTo']);
			die();
		} else {
			# if the database insertion failed, send them to the error page with a useful message
			errorPage('An error has occured while trying to add this Phone Type.');
		}
	}
...
<script language="javascript" type='text/javascript'>
	...
	function error_check() {
		...
		if(errEmpty('chrPhoneType', "You must enter a Phone Type Name.")) { totalErrors++; }
		
		if(document.getElementById('bShow0').checked == false && document.getElementById('bShow1').checked == false) {
			errCustom('',"You must choose either to Show or not to Show this Phone Type.");
			totalErrors++;
		}
...
<?	
	...
	$bodyParams = "document.getElementById('chrPhoneType').focus()";

	$section = 'admin';
	$leftlink = "phonetypes";
	include($BF .'includes/top.php');
	// Banner Information
	$banner_title = "Add Phone Type"; // Title of this page. (REQUIRED)
	$banner_icon = ""; // Icon for this page, Size MUST be 40x40 pixels. (NOT REQUIRED)
	$banner_xtra = ""; // Extra information for Page. (NOT REQUIRED)
	$banner_instructions = "You are adding a Phone Type to the database."; // Instructions or description. (NOT REQUIRED)
		...
		<table width="100%" class="twoCol800" id="twoCol800" cellpadding="0" cellspacing="0">
			<tr>
				<td class="left">
					...
					<div class='FormName'>Phone Type Name <span class='FormRequired'>(Required)</span></div>
					<div class='FormField'><input type="text" name="chrPhoneType" id="chrPhoneType" maxlength="150" size="50" /></div>
					
				</td>
				<td class="gutter"></td>
				<td class="right">

					<div class='FormName'>Show this Phone Type <span class='FormRequired'>(Required)</span></div>
...
?>
		<tr>
			...
			<td class="right"><input type='submit' value='Add Another' onclick="document.getElementById('moveTo').value='addphonetype.php';" /> &nbsp;&nbsp; <input type='submit' value='Add and Continue' onclick="document.getElementById('moveTo').value='phonetypes.php';" /></td>			
