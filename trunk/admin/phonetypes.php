<?
	...
	$q = "SELECT ID, chrKEY, chrPhoneType, bShow, intOrder
		FROM PhoneTypes
		WHERE !bDeleted
		ORDER BY intOrder,chrPhoneType";
	$results = db_query($q,"getting Phone Types");
...
?><script language="JavaScript" type='text/javascript' src="<?=$BF?>includes/overlays.js"></script><?	
	...
	// Banner Information
	$banner_title = "Phone Types"; // Title of this page. (REQUIRED)
	...
	$banner_instructions = "This is a list of Phone Types used by the system."; // Instructions or description. (NOT REQUIRED)

	include($BF .'includes/left_admin.php');

	# This is the include file for the overlay to show that the delete is working.
	$TableName = "PhoneTypes"; #  This is the Database table that you will be setting the bDeleted statuses on.
...
<? $count=0;
	...
	while ($row = mysqli_fetch_assoc($results)) { 
		$idTypes[] = $row['ID'];
		$link = 'window.location.href="editphonetype.php?key='.$row['chrKEY'].'"'; 
?>
			<tr id='tr<?=$row['ID']?>' class='<?=($count++%2 ? 'ListLineOdd' : 'ListLineEven')?>' 
				onmouseover='RowHighlight("tr<?=$row['ID']?>");' onmouseout='UnRowHighlight("tr<?=$row['ID']?>");'>
				<td onclick='<?=$link?>'><?=$row['chrPhoneType']?></td>
				<td class="NoCursor"><input type="text" size="3" name="intOrder<?=$row['ID']?>" id="intOrder<?=$row['ID']?>" value="<?=$row['intOrder']?>" /></td>
				<td onclick="javascript:showHide('<?=$BF?>',<?=$row['ID']?>,'<?=$TableName?>',document.getElementById('bShow<?=$row['ID']?>').value,<?=$_SESSION['idPerson']?>,'<?=$row['chrKEY']?>');" align="center" />
				...
				<td class='options'><a title="Edit <?=$row['chrPhoneType']?>" href='editphonetype.php?key=<?=$row['chrKEY']?>'><img src='<?=$BF?>images/edit.png' alt='edit' /></a></td>
				<td class='options'><?=deleteButton($row['ID'],$row['chrPhoneType'],$row['chrKEY'])?></td>
			</tr>
<?	} 
if($count == 0) { ?>
			<tr>
				<td align="center" colspan='5' height="20">No Phone Types have been created at this time.</td>
			</tr>
<?	} ?>
	</table>
<? if ($count != 0) { ?>
...
?>
		<tr>
			...
			<td class="addbutton"><a title="Add Phone Type" href="addphonetype.php"><img src="<?=$BF?>images/plus_add.gif" alt="Add Phone Type" /></a></td>
