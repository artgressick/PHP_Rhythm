<?php
	...
	if(!isset($_REQUEST['key'])) {
		errorPage('Invalid Phone Type');
	}

	# Getting all original data from this record
	$info = db_query("SELECT * FROM PhoneTypes WHERE chrKEY='". $_REQUEST['key']. "'","getting Phone Type info",1);
	if($info['ID'] == "") { errorPage('Invalid Phone Type'); }
	...
	if(isset($_POST['chrPhoneType'])) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		list($mysqlStr,$audit) = set_strs($mysqlStr,'chrPhoneType',$info['chrPhoneType'],$audit,$table,$info['ID']);
		list($mysqlStr,$audit) = set_strs($mysqlStr,'bShow',$info['bShow'],$audit,$table,$info['ID']);
		
		// if nothing has changed, don't do anything.  Otherwise update / audit.
		if($mysqlStr != '') { 
			$_SESSION['InfoMessage'] = $_POST['chrPhoneType']. " has been successfully updated in the Database.";
			list($str,$aud) = update_record($mysqlStr, $audit, $table, $info['ID']);
		} else {
			$_SESSION['InfoMessage'] = "No Changes have been made to ".$info['chrPhoneType'];
		}
		
		
		header("Location: phonetypes.php");
		die();	
	}
...
<?	
	...
	// Banner Information
	$banner_title = "Edit Phone Type:"; // Title of this page. (REQUIRED)
	$banner_icon = ""; // Icon for this page, Size MUST be 40x40 pixels. (NOT REQUIRED)
	$banner_xtra = $info['chrPhoneType']; // Extra information for Page. (NOT REQUIRED)
		...
		<table width="100%" class="twoCol800" id="twoCol800" cellpadding="0" cellspacing="0">
			<tr>
				<td class="left">
					...
					<div class='FormField'><input type="text" name="chrPhoneType" id="chrPhoneType" maxlength="150" size="50" value='<?=$info['chrPhoneType']?>' /></div>
