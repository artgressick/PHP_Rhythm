<?php
	...
	if(isset($_POST['idPerson'])) { // When doing isset, use a required field.  Faster than the php count funtion.
		...
		$table = 'CustomerAccess'; # added so not to forget to change the insert AND audit
		...
		if(db_query($q,"Insert into ". $table)) {
			...
			$_SESSION['InfoMessage'] = "Customer Access has been granted successfully.";
			...
			if($info['chrPassword'] == "") {
				...
				$info['txtMsg'] = "<p>Dear ".$info['chrFirst']." ".$info['chrLast'].",</p>
						<p>This E-mail is to notify you that you have been granted customer access to the Rhythm Business Portal. 
						...
						<p>Login At: <a href='http://my.techitsolutions.com/customers/index.php'>http://my.techitsolutions.com/customers/index.php</a></p>
			...
			errorPage('An error has occured while trying to add customer access.');
		}
	}
	
	$title = 'Add Person to Customer';
...
<?	
	...
	// Banner Information
	$banner_title = "Add Person to Customer"; // Title of this page. (REQUIRED)
	...
	$banner_instructions = "You are adding a person to have access to a customer."; // Instructions or description. (NOT REQUIRED)
...
<?	$q = "SELECT ID,chrFirst,chrLast
		FROM People
		WHERE !bDeleted
			AND People.ID NOT IN (SELECT ppl.ID FROM People as ppl JOIN CustomerAccess ON CustomerAccess.idPerson=ppl.ID WHERE !bDeleted)
...
?>
		<tr>
			...
			<td class="right"><input type='submit' value='Add Another' onclick="document.getElementById('moveTo').value='addcustomeraccess.php';" /> &nbsp;&nbsp; <input type='submit' value='Add and Continue' onclick="document.getElementById('moveTo').value='customeraccess.php';" /></td>			
